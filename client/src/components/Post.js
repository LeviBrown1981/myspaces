...
class Post extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(`/api/departments/${department_id}/posts/${id}`)
      .then(res => {
  ...
  handleDelete = () => {
    ...
    axios.delete(`/api/departments/${department_id}/posts/${id}`)
      .then(res => {
  ...
  render() {
    const { match: { params: { id, department_id } } } = this.props
    const { name, description, price } = this.state.post
    return (
      <Container style={{ marginBottom: '40px' }}>
        ...
        <p>{description}</p>
        <Link to={`/departments/${department_id}/posts/${id}/edit`}>
